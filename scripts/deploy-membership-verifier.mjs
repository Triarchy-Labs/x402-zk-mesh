...
const DEFAULT_WASM =
  "contracts/target/wasm32v1-none/release/verifier_membership.wasm";
...
function requireSecret() {
  const secret =
    ...
    (process.env.ZK_GUILD_REGISTRY_RELAYER?.startsWith("S")
      ...
      : "");
  if (!secret) {
    throw new Error("Set ZK_VERIFIER_RELAYER_SECRET or ZK_GUILD_REGISTRY_RELAYER_SECRET.");
  }
  return secret;
}
...
if (!fs.existsSync(wasmPath)) {
  throw new Error(`Missing membership verifier wasm at ${wasmPath}. Run npm run contracts:build first.`);
}
...
console.log("\n.env.local update:");
console.log(`ZK_VERIFIER_MEMBERSHIP=${contractId}`);
console.log("ZK_VERIFIER_RELAYER_SECRET=<keep-your-existing-secret>");
