			...
			<div style={{ maxWidth: "1200px", margin: "0 auto", padding: "120px 2rem 4rem" }}>
				...
				{loading ? (
					...
					<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
						{/* Table Header */}
						<div style={{ display: "grid", gridTemplateColumns: "60px 2fr 1fr 1fr 1fr 1fr 100px", padding: "0 1.25rem", color: "rgba(255,255,255,0.35)", fontSize: "0.7rem", letterSpacing: "0.1em", fontFamily: "'Space Mono', monospace" }}>
						...
						{data.leaderboard.map((entry, i) => (
							<motion.div
								...
								style={{
									display: "grid", gridTemplateColumns: "60px 2fr 1fr 1fr 1fr 1fr 100px",
							...
							>
								{/* Position */}
								<span style={{ fontFamily: "'Space Mono', monospace", color: i < 3 ? "#a855f7" : "rgba(255,255,255,0.4)", fontSize: i < 3 ? "1.2rem" : "1rem", fontWeight: i < 3 ? "bold" : "normal" }}>
									#{entry.position}
								</span>
								...
								<div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
									<div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
										<span style={{ fontSize: "1rem", fontWeight: "500", color: "#fff" }}>{entry.name}</span>
										<span style={{ fontSize: "0.65rem", padding: "2px 6px", background: entry.type === "bot" ? "rgba(0,191,255,0.2)" : "rgba(0,255,65,0.2)", color: entry.type === "bot" ? "#00bfff" : "#00ff41", borderRadius: "3px" }}>
											{entry.type.toUpperCase()}
										</span>
									</div>
									<div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
										<span style={{ fontSize: "0.75rem", fontFamily: "'Space Mono', monospace", color: RANK_COLORS[entry.rank] || "#fff" }}>
											[{entry.rank}]
										</span>
								...
								{/* Status */}
								<span style={{ textAlign: "right", fontSize: "0.75rem", color: entry.status === "active" ? "#00ff41" : entry.status === "busy" ? "#ffd700" : "rgba(255,255,255,0.3)", textTransform: "uppercase" }}>
									{entry.status}
								</span>
							</motion.div>
						))}
						...
						<div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
							{data.ranking_info.ranks.map((r) => (
								<div key={r.name} style={{ display: "grid", gridTemplateColumns: "150px 100px 1fr", alignItems: "center", padding: "0.5rem 0", borderBottom: "1px dashed rgba(255,255,255,0.1)" }}>
									<span style={{ color: RANK_COLORS[r.name], fontFamily: "'Space Mono', monospace", fontSize: "0.85rem", fontWeight: "bold" }}>{r.name}</span>
									<span style={{ color: "rgba(255,255,255,0.5)", fontFamily: "'Space Mono', monospace", fontSize: "0.8rem" }}>{r.xp_required.toLocaleString()} XP</span>
									<div style={{ display: "flex", gap: "0.5rem" }}>
										{r.task_access.map(tier => (
											<span key={tier} style={{ fontSize: "0.65rem", padding: "2px 6px", background: "rgba(255,255,255,0.05)", borderRadius: "3px", color: "rgba(255,255,255,0.6)" }}>{tier}-TIER</span>
										))}
									</div>
								</div>
							))}
